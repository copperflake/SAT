 ...
 * Un client radio.
 * <p>
 * Un client radio est capable de se connecter à un seveur
 * radio pour communiquer.
 * <p>
 * À la différence d'un serveur radio, le client
 * radio ne peut pas accepter de connexion entrante et n'est pas capable
 * de se connecter à plus d'un hôte.
 */
public class RadioClient extends Radio implements RadioClientEngineDelegate {
	/**
	 * Le délégué de la radio. Le délégué reçoit les évenements émis par
	 * la radio et est chargé de leur gestion.
	 */
	 ...
	 * 
	 * @param delegate	Le délégué chargé de la gestion des événements.
	 */
	...
	/**
	 * Connecte le client radio à un serveur radio en utilisant le moteur
	 * de communcation <code>engine</code> spécifié. Le moteur passé
	 * en paramètre est automatiquement initialisé.
	 * 
	 * @param engine		Le moteur de communication utilisé pour la connexion
	 * 
	 * @throws IOException	L'initialisation du moteur de communication peut
	 * 						provoquer une exception qui est passée au code
	 * 						appelant. De plus, si cette radio est déjà associée
	 * 						à un moteur de communication, une exception sera
	 * 						levée.
	 */
